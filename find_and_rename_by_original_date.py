#!/usr/bin/env python3
"""
Find the TRUE original date from metadata and rename files accordingly.
NEVER overwrites metadata - only reads it to find the original creation date.
Prioritizes: DateTimeOriginal > DateTimeDigitized > DateTime > File creation date
Only uses dates between 2001-2025.
"""
    ...
    try:
        with Image.open(image_path) as img:
            ...
            if exifdata:
                # Tag IDs for artist:
                # 315 = Artist
                # 305 = Software (sometimes contains photographer name)
...
def get_video_creation_date(video_path):
    """
    Get creation date from video file using exiftool or mdls.
    Returns: (datetime_object, source_name) or (None, None)
    """
    dates_found = {}
    
    # Try exiftool first (more reliable for videos)
    try:
        ...
        if result.returncode == 0:
            ...
            for line in lines:
                if line:
                    # exiftool output format: "2023:12:25 14:30:52"
    ...
    # Try mdls (macOS metadata)
    try:
        result = subprocess.run(
            ['mdls', '-name', 'kMDItemContentCreationDate', str(video_path)],
            capture_output=True,
            text=True,
            timeout=10
        )
        ...
        if result.returncode == 0:
            # Output format: kMDItemContentCreationDate = 2023-12-25 14:30:52 +0000
            output = result.stdout.strip()
            if '=' in output:
                date_str = output.split('=', 1)[1].strip()
                parsed_date = parse_mdls_date(date_str)
                if parsed_date and is_valid_date(parsed_date):
                    return parsed_date, 'Video Content Creation Date'
    except (subprocess.TimeoutExpired, FileNotFoundError):
        pass
...
def parse_mdls_date(date_str):
    """Parse mdls date string (macOS metadata)."""
    if not date_str:
        return None
    
    # Format: "2023-12-25 14:30:52 +0000"
    try:
        # Remove timezone info for simplicity
        date_str = date_str.split('+')[0].split('-', 3)[-1].strip()
...
def get_file_creation_date(file_path):
    ...
    try:
        ...
        # macOS has birthtime (true creation date)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Find original dates from metadata and rename files accordingly (NEVER overwrites metadata)'
    )
    ...
    print(f"{'='*80}")
    print(f"METADATA-BASED FILE RENAMER")
    print(f"{'='*80}")
    print(f"Directory: {source_dir}")
    print(f"Mode: {'DRY RUN (no changes will be made)' if args.dry_run else 'LIVE (files will be renamed)'}")
    print(f"Recursive: {'Yes' if args.recursive else 'No'}")
    print(f"\nThis script will:")
    print(f"  1. Find the TRUE original date from metadata")
    print(f"  2. Prioritize: DateTimeOriginal > DateTimeDigitized > DateTime > File creation")
    print(f"  3. Only use dates between 2001-2025")
    print(f"  4. NEVER overwrite or modify metadata")
    print(f"  5. Rename files based on original date found")
    print(f"{'='*80}\n")
