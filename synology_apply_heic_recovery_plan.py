#!/usr/bin/env python3
"""
Synology-optimized HEIC Recovery Plan Applier

Applies recovery plan to update HEIC file metadata using exiftool on Synology NAS.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Apply HEIC recovery plan on Synology (MODIFIES FILES!)'
    )
    parser.add_argument('--plan', default='/volume1/photo/recovery_plan_heic_only.csv',
    ...
    parser.add_argument('--log-file', default='/volume1/photo/logs/heic_recovery_apply.log',
                       help='Log file for changes')
    parser.add_argument('--undo-file', default='/volume1/photo/undo_heic_recovery.json',
                       help='Undo information file')
    
    args = parser.parse_args()
    
    print(f"{'='*80}")
    print(f"SYNOLOGY HEIC RECOVERY PLAN APPLIER")
    print(f"{'='*80}")
    ...
    if not check_exiftool():
        ...
        print("Install exiftool on Synology:")
        print("  1. SSH to Synology")
        print("  2. Run: sudo synopkg install exiftool")
        print("  Or download from: https://exiftool.org/install.html")
        return 1
