#!/usr/bin/env python3
"""
Safe Recovery Plan Creator - Phase 1 (READ ONLY)

Analyzes recovery logs and current state to build a recovery plan.
...
Assigns confidence levels based on how many sources agree.
"""
...
def extract_date_from_filename(filename):
    ...
    if match:
        try:
            ...
            if 2001 <= year <= 2025:
                return datetime(year, month, day, 12, 0, 0)  # Default noon
        except ValueError:
            pass
...
def extract_year_from_path(directory):
    """Extract year from directory path like '/volume1/photo/2018/'."""
...
def calculate_confidence(old_date, dir_year, modified_date, exif_date):
    ...
    # Check old filename date vs directory year
    ...
    # Check old filename date vs modified date
    ...
    # Check directory year vs modified year
    ...
    # Check if current EXIF is obviously wrong
    if exif_date:
...
def load_recovery_database(csv_path):
    """Load the recovery database (old → new filename mapping)."""
    recovery_map = {}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Create safe recovery plan (READ ONLY - no files modified)'
    )
    parser.add_argument('--recovery-db', required=True,
                       help='Path to recovery_list_*.csv')
    parser.add_argument('--state-capture', required=True,
                       help='Path to file_state_*.csv')
    parser.add_argument('--output', default='recovery_plan.csv',
                       help='Output recovery plan CSV (default: recovery_plan.csv)')
    ...
    print(f"{'='*80}")
    print(f"SAFE RECOVERY PLAN CREATOR (READ ONLY)")
    print(f"{'='*80}")
    ...
    for current_filename, state in state_map.items():
        ...
        # Parse current EXIF date (if any)
        exif_date = None
        ...
        # Determine if date needs updating
        # YES if: no EXIF data (NONE) OR EXIF differs from proposed by more than 1 day
        needs_update = False
        if not exif_date:
            needs_update = True  # No EXIF = needs to be populated
        elif abs((exif_date - old_date).days) > 1:
            needs_update = True  # EXIF differs significantly
        ...
        plan_entry = {
            ...
            'update_reason': 'No EXIF data' if not exif_date else ('EXIF differs by {} days'.format(abs((exif_date - old_date).days)) if needs_update else 'EXIF already correct')
        }
    ...
    print(f"{'='*80}")
    print(f"\n✅ NEXT STEPS:")
    print(f"1. Open {args.output} in Excel and review")
    print(f"2. Look for HIGH confidence entries - these are safest")
    print(f"3. Check 'reasoning' column to understand why each date was chosen")
    print(f"4. When ready, use apply_recovery_plan.py to update metadata")
    print(f"{'='*80}")
